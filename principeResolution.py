...
def negation(F):
    return [[-literal if isinstance(literal, int) else -literal[0] for literal in clause] for clause in F]
            ...
            for literal_i in clauses[i]:
                for literal_j in clauses[j]:
                    if abs(literal_i) == abs(literal_j):  
                        ...
                        if set(resolvant) not in set(map(tuple, result)):  
...
def resolution(F):
    ...
    while True:
        ...
        for resolvant in new_resolvantes:
            if set(resolvant) not in set(map(tuple, clauses)):  
...
def test_resolution():
    formula_input = input("Enter the formula : ")
